...
sub epilog {
    ...
    if ( $pl->level() == 0 ) {
        ...
        #my @users = keys %{ driver()->password() } ;
        #$pl->level( getsetup('LevelGod') ) unless scalar @users;
        my $dbh = dbi();
        my $sql = qq[ select count(1) from engine_password ];
...
sub display_step1 { 
    ...
    push @outp, ( parse_color("   {B}si` o no (".std_msg('yes')."/".std_msg('no').") ?{/B}\n"));
    push @outp, ( "> ");
    write_client( @outp );
}
...
sub display_step6 { 
    ...
    push @outp, ( "Hai un monitor in bianco e nero.\n" ) unless $pl->ansi_color;
    push @outp, ( parse_color("Hai un monitor a " .
                  "{Red}c{Yellow}o{Green}lo{Blue}ri{NORMAL}.\n" ) ) if $pl->ansi_color;
    push @outp, ( "Hai accettato le regole di questo MUD.\n" ) ;
    push @outp, ( "\n" );
